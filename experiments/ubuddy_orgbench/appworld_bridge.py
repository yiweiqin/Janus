...
class Bridge:
    ...
    def reset(self, task_id: str, experiment_name: str) -> dict[str, Any]:
        ...
        timeout_seconds = max(1, int(os.environ.get("UBUDDY_ORGBENCH_EXECUTION_TIMEOUT_SEC", "45")))
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            self.world = self.AppWorld(task_id=task_id, experiment_name=experiment_name, timeout_seconds=timeout_seconds)
        return {
            "taskId": task_id,
    ...
    def execute(self, code: str, role: str) -> dict[str, Any]:
        ...
        def _timeout(_signum: int, _frame: Any) -> None:
            raise TimeoutError(f"appworld_execution_timeout:{timeout_seconds}s")
