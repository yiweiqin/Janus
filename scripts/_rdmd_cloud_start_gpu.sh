#!/usr/bin/env bash
# 本机工具（下划线前缀）：把 _rdmd_cloud_start.sh 以 gpu_worker 后端起一遍。
# 为什么不改原脚本的默认值：null 后端的全链路验收（恒定 record_only、不伪造出处）
# 是 P4 的一条独立验收项，默认值就该留给它。这里只是在外层先导出 RDMD_BACKEND，
# 原脚本的 \ 会照常读到它 —— 一行不改，两种后端都能起。
export RDMD_BACKEND=gpu_worker
